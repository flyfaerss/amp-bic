...
class CombinedROIHeads(torch.nn.ModuleDict):
    ...
    def forward(self, features, proposals, targets=None, logger=None, loss_weight=1, predicate_weight=None):
        losses = {}
        x, detections, loss_box = self.box(features, proposals, targets)
        num_prop = [len(detection) for detection in detections]
        # print(num_prop, (0 in num_prop) or (1 in num_prop))
        ...
        if self.cfg.MODEL.RELATION_ON:
            ...
            # During training, self.box() will return the unaltered proposals as "detections"
            # this makes the API consistent during training and testing
            x, detections, loss_relation = self.relation(features, detections, targets, logger, loss_weight, predicate_weight)
