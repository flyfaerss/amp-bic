...
class RelationLossComputation(object):
    ...
    def __call__(self, proposals, rel_labels, relation_logits, refine_logits, weight=None):
        """
        Computes the loss for relation triplet.
        ...
        '''masked_bg_rel_num = max(len(rel_labels) - 4 * len(torch.nonzero(rel_labels)), 0)
        bg_index = torch.nonzero(rel_labels == 0).squeeze()
        shuffle_bg_index = bg_index[torch.randperm(len(bg_index))]
        masked_bg_index = shuffle_bg_index[:masked_bg_rel_num]
        rel_labels[masked_bg_index] = -1'''
        if len(torch.nonzero(rel_labels)) == 0:
            loss_relation = None
        else:
            if weight is None:
                loss_relation = self.criterion_loss(relation_logits[rel_labels != -1],
                                            rel_labels[rel_labels != -1].long())
            else:
                criterion_loss = nn.CrossEntropyLoss(weight=weight)
                loss_relation = criterion_loss(relation_logits[rel_labels != -1],
                                            rel_labels[rel_labels != -1].long())

                #represent_loss_relation
...
class RelationContrastLossComputation(object):
    """
    Computes the contrast loss for relation triplet.
    """
    def __init__(
        self,
        use_label_smoothing,
    ...
    def __call__(self, proposals, rel_labels, relation_logits, refine_logits, instance_embed, relation_embed):
...
def make_roi_relation_contrast_loss_evaluator(cfg):

    contrast_loss_evaluator = RelationContrastLossComputation(
        cfg.MODEL.ROI_RELATION_HEAD.LABEL_SMOOTHING_LOSS,
    )

    return contrast_loss_evaluator
