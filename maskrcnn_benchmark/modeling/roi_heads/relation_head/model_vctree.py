...
class DecoderTreeLSTM(torch.nn.Module):
    def __init__(self, cfg, classes, embed_dim, inputs_dim, hidden_dim, direction='backward', dropout=0.2):
        ...
        if direction == 'backward':
            ...
            self.decoderLSTM = BiTreeLSTM_Backward(self.input_size, self.hidden_size, is_pass_embed=True,
                                                   embed_layer=self.obj_embed, embed_out_layer=self.out)
        elif direction == 'foreward':
            self.input_size = inputs_dim + embed_dim * 2
            self.decoderLSTM = BiTreeLSTM_Foreward(self.input_size, self.hidden_size, is_pass_embed=True,
                                                   embed_layer=self.obj_embed, embed_out_layer=self.out)
        else:
...
class VCTreeLSTMContext(nn.Module):
    ...
    def __init__(self, config, obj_classes, rel_classes, statistics, in_channels):
        ...
        self.model_flag = self.cfg.MODEL.ROI_RELATION_HEAD.PREDICT_USE_BIAS
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
        ...
        obj_ctxs, obj_preds, obj_dists = self.obj_ctx(num_objs, obj_pre_rep, proposals, obj_labels, vc_forest,
    ...
    def vctree_score_net(self, num_objs, roi_feat, roi_dist, proposals):
        ...
        for sub, obj, dist, prp in zip(sub_feats, obj_feats, roi_dists, proposals):
            ...
            if not self.model_flag:
                ...
                #    cat([sub * obj, sub, obj, co_prior.unsqueeze(-1)], dim=-1).view(num_obj * num_obj, -1)).view(num_obj,
                #                                                                                                 num_obj)
                vis_prior = self.vision_prior(
                    cat([sub * obj, sub, obj], dim=-1).view(num_obj * num_obj, -1)).view(num_obj,
                                                                                                                 num_obj)
