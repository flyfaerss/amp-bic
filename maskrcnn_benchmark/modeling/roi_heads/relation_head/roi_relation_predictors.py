...
from maskrcnn_benchmark.modeling.roi_heads.relation_head.model_msdn import MSDNContext
from maskrcnn_benchmark.modeling.roi_heads.relation_head.model_naive import (
...
from .model_fa import PISAContext
# from .model_faop import PISAContext
# from .model_ffcls import PISAContext
# from .model_fapcpl import PISAContext
from .model_afe import AFEContext
# from .model_afe_temp import AFEContext
from .model_bgnn import BGNNContext
# from .model_faop import BGNNContext
...
class TransformerPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.embed_dim = 300
        self.contrast_loss = cfg.MODEL.ROI_RELATION_HEAD.PISA_MODULE.USE_CONTRAST_LOSS

        if self.use_glove_prototype:
            self.rel_feats_embed = make_fc(self.pooling_dim, self.embed_dim)
        else:
            self.rel_feats_embed = make_fc(self.pooling_dim, self.hidden_dim)

        if self.contrast_loss:
            self.feat_loss_eval = FeatureLoss(config)
    ...
    def forward(
        ...
        rel_feats_embed = self.rel_feats_embed(visual_rep)
        ...
        if self.training and self.contrast_loss:
            rel_labels = cat(rel_labels, dim=0)
            if len(squeeze_tensor(torch.nonzero(rel_labels != -1))) == 0:
                loss_rel_pre_cls = None
            else:
                # loss_rel_pre_cls = self.rel_aware_loss_eval(pre_cls_logits, rel_labels)
                loss_rel_pre_cls = self.feat_loss_eval(rel_feats_embed, rel_labels, self.relation_embed.to(rel_feats_embed.device), predicate_weight)
            if loss_weight is not None:
                add_losses[f"loss_rel_feature"] = loss_rel_pre_cls * (1 - loss_weight)
            else:
                add_losses[f"loss_rel_feature"] = loss_rel_pre_cls
...
class BGNNPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.split_context_model4inst_rel = (
            config.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.SPLIT_GRAPH4OBJ_REL # False
        )
        if self.split_context_model4inst_rel:
            ...
            self.context_layer = BGNNContext(
                ...
                self.input_dim,
                hidden_dim=self.hidden_dim,
                num_iter=config.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.GRAPH_ITERATION_NUM, # 3
            )
    ...
    def forward(
        ...
        if not self.training:
            self.feature_storage.append(np.array(rel_feats.cpu()))
            self.feature_count += 1
            if self.feature_count == 26446:
                with open(os.path.join(cfg.OUTPUT_DIR, 'feature.pkl'), 'wb') as f:
...
@registry.ROI_RELATION_PREDICTOR.register("AFEPredictor")
class AFEPredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(AFEPredictor, self).__init__()
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES # 151
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES # 51
        self.use_bias = config.MODEL.ROI_RELATION_HEAD.FREQUENCY_BAIS # true
        ...
        self.pooling_dim = cfg.MODEL.ROI_RELATION_HEAD.CONTEXT_POOLING_DIM # 2048
        self.input_dim = in_channels
        self.hidden_dim = config.MODEL.ROI_RELATION_HEAD.PISA_MODULE.GRAPH_HIDDEN_DIM # 512
        ...
        self.rel_feature_type = config.MODEL.ROI_RELATION_HEAD.EDGE_FEATURES_REPRESENTATION
        self.use_relness_loss = config.MODEL.ROI_RELATION_HEAD.PISA_MODULE.USE_RELNESS_LOSS
        self.use_glove_prototype = cfg.MODEL.ROI_RELATION_HEAD.PISA_MODULE.USE_GLOVE_PROTOTYPE
        ...
        self.embed_dim = 300
        if self.use_glove_prototype:
            self.rel_feats_embed = make_fc(self.hidden_dim, self.embed_dim)
        else:
            self.rel_feats_embed = make_fc(self.hidden_dim, self.hidden_dim)
        if self.use_obj_recls_logits:
            self.inst_feats_embed = make_fc(self.hidden_dim, self.embed_dim)

        statistics = get_dataset_statistics(config)

        self.relness_loss_eval = RelnessLoss(config)
        self.pooling_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_POOLING_DIM
        self.contrast_loss = cfg.MODEL.ROI_RELATION_HEAD.PISA_MODULE.USE_CONTRAST_LOSS
    ...
    def forward(
        ...
        obj_feats, rel_feats, relation_embed, relatedness = self.context_layer(
            roi_features, union_features, inst_proposals, rel_pair_idxs, rel_labels, logger
        )
        ...
        if relatedness is not None and not self.training:
            split_relatedness = relatedness.split(num_rels, dim=0)
            for idx, (prop, rel_pair, relness) in enumerate(zip(inst_proposals, rel_pair_idxs, split_relatedness)):
                pred_rel_matrix = torch.zeros(
                    (len(prop), len(prop)), device=relatedness.device, dtype=relatedness.dtype
                )
                pred_rel_matrix[rel_pair[:, 0], rel_pair[:, 1]] = relness
                prop.add_field("relness_mat", pred_rel_matrix)
        ...
        if self.use_obj_recls_logits:
            if self.mode == "sgdet" and not self.training:
                ...
                obj_pred_labels = refined_obj_logits[:, 1:].max(-1)[1] + 1
                obj_pred_logits = refined_obj_logits
        else:
            obj_pred_labels = cat(
                [each_prop.get_field("pred_labels") for each_prop in inst_proposals], dim=0
        ...
        if self.use_bias:
            ...
            pair_pred = cat(pair_preds, dim=0)
            freq_dist_bias = self.freq_bias.index_with_labels(pair_pred.long())
            rel_cls_logits = rel_cls_logits + freq_dist_bias
        ...
        if relatedness is not None and self.training and self.use_relness_loss:
            rel_labels = cat(rel_labels, dim=0)
            if len(squeeze_tensor(torch.nonzero(rel_labels != -1))) == 0:
                loss_relness = None
            else:
                loss_relness = self.relness_loss_eval(relatedness, rel_labels)

            add_losses[f"loss_relness"] = loss_relness

        if self.training and self.contrast_loss:
            if not self.use_relness_loss:
                rel_labels = cat(rel_labels, dim=0)
            if len(squeeze_tensor(torch.nonzero(rel_labels != -1))) == 0:
                loss_rel_pre_cls = None
            else:
                # loss_rel_pre_cls = self.rel_aware_loss_eval(pre_cls_logits, rel_labels)
                loss_rel_pre_cls = self.feat_loss_eval(rel_feats_embed, rel_labels, relation_embed, predicate_weight)
            if loss_weight is not None:
                add_losses[f"loss_rel_feature"] = loss_rel_pre_cls * (1 - loss_weight)
            else:
                add_losses[f"loss_rel_feature"] = loss_rel_pre_cls
            '''if self.use_obj_recls_logits:
                loss_obj_pre_cls = self.feat_loss_eval(inst_feats_embed, obj_labels, instance_embed)
                add_losses[f"loss_obj_feature"] = loss_obj_pre_cls * (1 - loss_weight)'''

        return obj_pred_logits, rel_cls_logits, add_losses


@registry.ROI_RELATION_PREDICTOR.register("PISAPredictor")
class PISAPredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(PISAPredictor, self).__init__()
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES # 151
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES # 51
        self.use_bias = config.MODEL.ROI_RELATION_HEAD.FREQUENCY_BAIS # true
        ...
        self.context_layer = PISAContext(
            config,
            self.input_dim,
            hidden_dim=self.hidden_dim,
        )
        ...
        self.rel_feature_type = config.MODEL.ROI_RELATION_HEAD.EDGE_FEATURES_REPRESENTATION
        self.use_relness_loss = config.MODEL.ROI_RELATION_HEAD.PISA_MODULE.USE_RELNESS_LOSS
        self.use_informative_loss = config.MODEL.ROI_RELATION_HEAD.PISA_MODULE.USE_INFORMATIVE_LOSS
        self.use_glove_prototype = cfg.MODEL.ROI_RELATION_HEAD.PISA_MODULE.USE_GLOVE_PROTOTYPE
        ...
        statistics = get_dataset_statistics(config)
        self.pretrain_mode = config.MODEL.ROI_RELATION_HEAD.PISA_MODULE.PRETRAIN_MODE
        self.use_informative = config.MODEL.ROI_RELATION_HEAD.PISA_MODULE.IS_USE_INFORMATIVE
        self.predicate_frequency_dir = config.OUTPUT_DIR
        ...
        if self.use_informative:
            self.informative_loss_eval = InformativeLoss(config)
        ...
        self.pooling_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_POOLING_DIM
        self.contrast_loss = cfg.MODEL.ROI_RELATION_HEAD.PISA_MODULE.USE_CONTRAST_LOSS
        self.transmit_informative_loss = cfg.MODEL.ROI_RELATION_HEAD.PISA_MODULE.TRANS_LOSS
        self.seperate_training = cfg.MODEL.ROI_RELATION_HEAD.PISA_MODULE.SEPERATE_TRAINING
    ...
    def get_distance(self, factor, distribution):
        ...
        for i in range(len(factor)):
            new_distribution.append(1 - (distribution - factor[i])**2)
    ...
    def forward(
        ...
        # relatedness : (batch_size, the number of object in an image, the number of object in an image, iteration:3)
        # this data is just for test
        obj_feats, rel_feats, relation_embed, relatedness, refine_distance, informative_factor = self.context_layer(
            roi_features, union_features, inst_proposals, rel_pair_idxs, rel_labels, logger
        )
        ...
        if self.use_informative:
            if self.transmit_informative_loss:
                rel_cls_logits_final = rel_cls_logits * refine_distance
            else:
                rel_cls_logits_final = rel_cls_logits * refine_distance.detach()

        obj_pred_logits = obj_pred_logits.split(num_objs, dim=0)
        if self.training:
            if not self.seperate_training and self.use_informative:
                rel_cls_logits = rel_cls_logits_final.split(num_rels, dim=0)
            else:
                rel_cls_logits = rel_cls_logits.split(num_rels, dim=0)
        else:
            if self.use_informative:
        ...
        if self.use_informative and self.training and self.use_informative_loss:
            if not self.use_relness_loss:
                rel_labels = cat(rel_labels, dim=0)
            loss_informative = self.informative_loss_eval(informative_factor.squeeze(), rel_labels,
                                                          self.frequency_distribution.to(dtype=informative_factor.dtype,
                                                                                         device=informative_factor.device))
            add_losses[f"loss_informative"] = loss_informative

        if self.training and self.contrast_loss:
            if not self.use_relness_loss and not self.use_informative_loss:
                rel_labels = cat(rel_labels, dim=0)
            if len(squeeze_tensor(torch.nonzero(rel_labels != -1))) == 0:
                loss_rel_pre_cls = None
            else:
            ...
            if loss_weight is not None:
                ...
                add_losses[f"loss_rel_feature"] = loss_rel_pre_cls
            '''if self.use_obj_recls_logits:
                loss_obj_pre_cls = self.feat_loss_eval(inst_feats_embed, obj_labels, self.instance_embed.to(inst_feats_embed.device))
                add_losses[f"loss_obj_feature"] = loss_obj_pre_cls * (1 - loss_weight)'''
...
class VCTreePredictor(nn.Module):
    ...
    def forward(
        ...
        temp = prod_rep * union_features
        rel_feats_embed = self.rel_feats_embed(temp)

        ctx_dists = self.ctx_compress(temp)
