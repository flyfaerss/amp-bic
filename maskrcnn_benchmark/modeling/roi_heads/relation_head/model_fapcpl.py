...
class MessagePassingUnit(nn.Module):
    def __init__(self, input_dim, pooling_dim):
        ...
        self.ww = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(input_dim * 2, pooling_dim, bias=True),
            nn.ReLU(),
            nn.Linear(pooling_dim, 1)
        )
        # self.eps = 0.3
...
class FALayer(nn.Module):
    def __init__(self, hidden_dim, dropout):
        ...
        self.gate = nn.Linear(2 * self.hidden_dim, 1)
    ...
    def forward(self, inst_feature, norm_degree, aggregator_matrix, rel_pair_index):
        ...
        new_inst_feature = torch.cat([new_obj_feature, new_sub_feature], dim=1)
        ...
        aggregator_factor[obj_index, sub_index] = g
...
class PISAContext(nn.Module):
    def __init__(
        ...
        if self.use_contrast_loss:
            ...
            self.instance_embed = Variable(torch.randn((self.num_obj_classes, self.hidden_dim)).cuda())
            self.instance_embed.requires_grad = True
            self.instance_count = torch.zeros(self.num_obj_classes)
            self.use_gt_label2feature = cfg.MODEL.ROI_RELATION_HEAD.PISA_MODULE.USE_GT_LABEL4FEATURE
        ...
        # self.informative_embed_dim = config.MODEL.ROI_RELATION_HEAD.PISA_MODULE.INFORMATIVE_EMBED_DIM
        weight = cfg.MODEL.ROI_RELATION_HEAD.PISA_MODULE.INFORMATIVE_WEIGHT
        self.informative_weight = nn.Parameter(torch.Tensor([weight]), requires_grad=False)
        ...
        '''self.gate_obj2obj = nn.Sequential(
            *[MessagePassingUnit(self.hidden_dim, gate_width) for _ in range(self.message_passing_unit_num)]
        )'''
        ...
        self.inst_fusion = MessageFusion(self.hidden_dim, dropout)

        self.inst_message_fusion = MessageFusion(self.hidden_dim, dropout)
        self.rel_message_fusion = MessageFusion(self.hidden_dim, dropout)
        '''self.inst_message_fusion = nn.Sequential(
            *[MessageFusion(self.hidden_dim, dropout) for _ in range(self.message_passing_unit_num)]
        )

        self.rel_message_fusion = nn.Sequential(
            *[MessageFusion(self.hidden_dim, dropout) for _ in range(self.message_passing_unit_num)]
        )'''
    ...
    def message_pred2inst(self, inst_feats, rel_feats, subj_pred_map, gate_module, relness_scores):
    ...
    def message_inst2inst(self, obj_feats, gate_module, rel_pair_index, relness_scores=None):
        ...
        if relness_scores is not None:
            select_rel_pair_index = rel_pair_index[relness_scores > 0.0001]
        else:
        ...
        raw_feature = obj_feats
        for i in range(self.layer_num):
            obj_feats, g = gate_module[i](obj_feats, norm_degree, aggregator_matrix, select_rel_pair_index)
            obj_feats = self.eps * raw_feature + obj_feats
        ...
        return feature_data, pred_finetune_relatedness
    ...
    def informative_predictor(self, inst_proposals, num_inst_list, inst_features, rel_pair_idxs):
        if self.mode == 'predcls':
            ...
            inst_pred_labels = torch.cat([each_prop.get_field("pred_labels").detach() for each_prop in inst_proposals],
                                        dim=0).split(num_inst_list, dim=0)
            inst_pred_logits = torch.cat([each_prop.get_field("pred_logits").detach() for each_prop in inst_proposals],
                                        dim=0).split(num_inst_list, dim=0)

        inst_features = inst_features.detach().split(num_inst_list, dim=0)
    ...
    def forward(
        ...
        for iter_num in range(self.message_prop_iter_num):
            ...
            for mp_iter in range(self.message_passing_unit_num):
                ...
                rel_fusion_feature = self.GRU_rel_fusion(add_rel2sub_feature, add_rel2obj_feature)
                inst_refine_feature = self.inst_message_fusion(rel_fusion_feature, inst_mp_iters[mp_iter],
                                                               inst_raw_feature)
                # inst_refine_feature = inst_refine_feature + inst_mp_iters[mp_iter]
                ...
                # valid_raw_sub_feature = torch.index_select(inst_feats_iters[iter_num], 0, sub_valid)
                # valid_raw_obj_feature = torch.index_select(inst_feats_iters[iter_num], 0, obj_valid)
                ...
                # print(iter_num, mp_iter, inst_mp_iters[mp_iter].shape, rel_mp_iters[mp_iter].shape, valid_rel_feature.shape, add_sub2rel_feature.shape, add_obj2rel_feature.shape)
                inst_fusion_feature = self.GRU_inst_fusion(add_sub2rel_feature, add_obj2rel_feature)
                valid_rel_refine_feature = self.rel_message_fusion(inst_fusion_feature, valid_rel_feature,
                                                                   valid_raw_rel_feature)
        ...
        return (inst_feats_final, rel_feats_final, None, pred_relatedness_score_iters[-1], refine_distance,
                informative_factor)
