...
class PairwiseFeatureExtractor(nn.Module):
    ...
    def __init__(self, config, in_channels):
        ...
        if self.rel_feature_type in ["fusion"]:
        ...
        self.obj_embed_pred_fusion = make_fc(2 * self.embed_dim, self.embed_dim)
    ...
    def forward(self, inst_roi_feats, union_features, inst_proposals, rel_pair_idxs, ):
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL:
            ...
            obj_embed_by_pred = self.obj_embed_on_prob_dist(obj_labels.long())
        else:
            ...
            obj_embed_by_pred = cat((obj_embed_by_pred_dist, obj_embed_by_pred_labels), -1)
            obj_embed_by_pred = self.obj_embed_pred_fusion(obj_embed_by_pred)

        assert inst_proposals[0].mode == 'xyxy'
        pos_embed = self.pos_embed(encode_box_info(inst_proposals))

        obj_pre_rep = cat((inst_roi_feats, obj_embed_by_pred, pos_embed), -1)
