...
class DecoderRNN(nn.Module):
    ...
    def forward(self, inputs, initial_state=None, labels=None, boxes_for_nms=None):
        ...
        end_ind = 0
        # do prediction on features after each LSTM step
        for i, l_batch in enumerate(batch_lengths):
            ...
            if self.training:
                ...
                labels_to_embed = labels_to_embed.type(nonzero_pred.type())
                is_bg = (labels_to_embed == 0).nonzero()
                # print(labels.type())
                # print(labels_to_embed.type())
                # print(nonzero_pred.type())
                # print(is_bg.type())
                if is_bg.dim() > 0:
        ...
        if boxes_for_nms is not None and not self.training:
            ...
            refined_obj_labels = refined_obj_labels[0].new(len(refined_obj_labels)).fill_(0)

            for i in range(refined_obj_labels.size(0)):
                box_ind, cls_ind = np.unravel_index(out_dists_sampled.argmax(), out_dists_sampled.shape)
                refined_obj_labels[int(box_ind)] = int(cls_ind)
                out_dists_sampled[is_overlap[box_ind, :, cls_ind], cls_ind] = 0.0
                out_dists_sampled[box_ind] = -1.0  # This way we won't re-sample

            refined_obj_labels = refined_obj_labels
        else:
            refined_obj_labels = torch.cat(refined_obj_labels, 0)

        return torch.cat(out_dists, 0), refined_obj_labels
...
class LSTMContext(nn.Module):
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
        ...
        if self.mode == 'sgdet' and not self.training:
            boxes_per_cls = cat([proposal.get_field('boxes_per_cls') for proposal in proposals],
                                dim=0)  # comes from post process of box_head
