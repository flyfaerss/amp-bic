...
class RelAwareRelFeature(nn.Module):
    def __init__(
        ...
        if self.visual_features_on:
            self.vis_embed = nn.Sequential(
                nn.ReLU(),
                nn.Linear(self.input_dim, self.hidden_dim), # input_dim : 2048 ; hidden_dim : 512
            )
    ...
    def forward(
        ...
        if self.visual_features_on:
            visual_feat = self.vis_embed(visual_feat.detach()) # （454， 2048) -> (454, 512)
        ...
        for img_id, (proposal, vis_feats, pair_idx) in enumerate(
            ...
            if self.predictor_type == "hybrid":
                ...
                relness_logits = torch.cat((relness_bin_logits, relness_logits), dim=1)
...
@registry.RELATION_CONFIDENCE_AWARE_MODULES.register("SpatialVisualBasedAware")
class SpatialVisualBasedAware(nn.Module):
    def __init__(self, input_dim):
        super(SpatialVisualBasedAware, self).__init__()
        self.cfg = cfg
        self.num_rel_cls = cfg.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        ...
        self.obj_geo_embed = nn.Sequential(
            make_fc(9, self.geometry_feat_dim),
            nn.ReLU(),
            make_fc(self.geometry_feat_dim, self.geometry_feat_dim),
        )

        self.visual_features_on = cfg.MODEL.ROI_RELATION_HEAD.RELATION_PROPOSAL_MODEL.VISUAL_FEATURES_ON

        if self.visual_features_on:
            self.inst_vis_embed = nn.Sequential(
                nn.ReLU(),
                make_fc(self.input_dim, self.hidden_dim), # input_dim : 2048 ; hidden_dim : 512
            )
            self.rel_vis_embed = nn.Sequential(
                nn.ReLU(),
                make_fc(self.input_dim, self.hidden_dim),  # input_dim : 2048 ; hidden_dim : 512
            )
        ...
        '''self.proposal_relness_geo_fc = nn.Sequential(
            nn.ReLU(),
            make_fc(self.geometry_feat_dim * 2, 1),
        )'''

        '''self.proposal_relness_vis_fc = nn.Sequential(
            nn.ReLU(),
            make_fc(self.hidden_dim * 2, 1),
        )'''

        self.proposal_relness_cls_fc = nn.Sequential(
            nn.LayerNorm(self.geometry_feat_dim * 2 + self.hidden_dim * 2),
            nn.ReLU(),
            make_fc(self.geometry_feat_dim * 2 + self.hidden_dim * 2, self.hidden_dim),
            nn.ReLU(),
            make_fc(self.hidden_dim, 1),
        )
    ...
    def forward(
        ...
        if self.visual_features_on:
            visual_inst_feat = self.inst_vis_embed(visual_inst_feat.detach())
            visual_rel_feat = self.rel_vis_embed(visual_rel_feat.detach()) # （454， 2048) -> (454, 512)

        visual_rel_feat_split = torch.split(visual_rel_feat, [len(p) for p in rel_pair_indexs], dim=0)
        visual_obj_feat_split = torch.split(visual_inst_feat, [len(q) for q in inst_proposals], dim=0)

        for img_id, (proposal, obj_vis_feats, rel_vis_feats, pair_idx) in enumerate(
            zip(inst_proposals, visual_obj_feat_split, visual_rel_feat_split, rel_pair_indexs)
        ):
            # pred_logits = proposal.get_field("predict_logits").detach()
            # obj_sem_embed = F.softmax(pred_logits, dim=1) @ self.obj_sem_embed.weight
            pred_rel_matrix = torch.zeros(
                (len(proposal), len(proposal)), device=visual_rel_feat.device, dtype=visual_rel_feat.dtype
            )

            geo_embed = self.obj_geo_embed(encode_box_info([proposal]))
            '''pair_geo_embed = torch.cat((
                geo_embed[pair_idx[:, 0]],
                geo_embed[pair_idx[:, 1]]),
                dim=1)

            pair_vis_embed = torch.cat((
                obj_vis_feats[pair_idx[:, 0]],
                obj_vis_feats[pair_idx[:, 1]]),
                dim=1)

            # obj_sem_embed = F.softmax(pred_logits, dim=1) @ self.obj_sem_embed.weight

            geo_relness = self.proposal_relness_geo_fc(pair_geo_embed)
            vis_relness = self.proposal_relness_vis_fc(pair_vis_embed)
            relness_scores = self.alpha * geo_relness + (1 - self.alpha) * vis_relness

            # relness_scores = self.proposal_relness_cls_fc(pair_geo_embed)
            relness_scores = squeeze_tensor(torch.sigmoid(relness_scores))'''
            ...
            pair_embed = torch.cat((
                ...
                obj_vis_feats[pair_idx[:, 1]]),
                dim=1)
            relness_scores = self.proposal_relness_cls_fc(pair_embed)
            relness_scores = squeeze_tensor(relness_scores)

            pred_rel_matrix[pair_idx[:, 0], pair_idx[:, 1]] = relness_scores

            relness_matrix.append(pred_rel_matrix)

        return None, relness_matrix
