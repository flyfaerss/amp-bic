...
class MessageFusion(nn.Module):
    ...
    def forward(self, input, hidden, raw=None):
        if raw is not None:
            output = self.wih(F.relu(input)) + self.whh(F.relu(hidden)) + self.wrh(F.relu(raw))
        else:
    ...
    def forward(
        ...
        for iter_num in range(self.message_prop_iter_num):
            ...
            for mp_iter in range(self.message_passing_unit_num):
                ...
                #rel_fusion_feature = self.eps * inst_raw_feature + rel_fusion_feature
                inst_refine_feature = self.inst_message_fusion(inst_mp_iters[mp_iter], rel_fusion_feature, inst_raw_feature)
                ...
                # inst_fusion_feature = self.eps * valid_raw_rel_feature + inst_fusion_feature
