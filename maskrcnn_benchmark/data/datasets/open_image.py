...
from maskrcnn_benchmark.data.datasets.visual_genome import resampling_dict_generation, get_VG_statistics, \
    apply_resampling
...
class OIDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, ann_file, cate_info_file, transforms=None,
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.DATA_RESAMPLING and self.split == 'train' and self.resampling_method in ['bilvl', 'lvis']:
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.DATA_RESAMPLING and self.split == 'train' and self.resampling_method == 'faip':
            # creat repeat dict in main process, other process just wait and load
            if get_rank() == 0:
                repeat_dict, rel_true_counter = resampling_dict_generation(self, self.ind_to_predicates, self.ind_to_classes, logger)
                self.repeat_dict = repeat_dict
            ...
            synchronize()
            self.repeat_dict, self.rel_true_counter = resampling_dict_generation(self, self.ind_to_predicates, self.ind_to_classes, logger)
    ...
    def get_statistics(self):
        if 'gqa' in cfg.DATASETS.TRAIN:
            rel_counter_init, inst_counter_init = get_dataset_statistics(self, must_overlap=True)
        ...
        cate_num = cfg.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        cate_num = cate_num - 1

        F_c = np.zeros(cate_num)
...
def get_dataset_statistics(train_data, must_overlap=True):
    ...
    return rel_counter, inst_counter
