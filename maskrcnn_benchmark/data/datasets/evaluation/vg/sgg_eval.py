...
def _compute_pred_matches(
    ...
    gt_inds, gt_boxeses, keep_indses = np.where(gt_has_match)[0], gt_boxes[gt_has_match], keeps[gt_has_match]
...
def match(gt_triplets, pred_triplets, gt_boxes, pred_boxes):
    keeps = intersect_2d(gt_triplets, pred_triplets)
    pred_list = []
    feature_list = []
    all_match = np.array([True] * len(gt_triplets)).reshape(-1)
    all_inds, all_boxeses, all_keep_indses = np.where(all_match)[0], gt_boxes[all_match], keeps[all_match]

    for all_ind, all_box, all_keep_inds in zip(all_inds, all_boxeses, all_keep_indses):
        boxes = pred_boxes
        sub_iou = bbox_overlaps(all_box[None, :4], boxes[:, :4])[0]
        obj_iou = bbox_overlaps(all_box[None, 4:], boxes[:, 4:])[0]
        # inds = (sub_iou >= iou_thres) & (obj_iou >= iou_thres)
        inds = (sub_iou >= 0.99) & (obj_iou >= 0.99)
        pred_list.append(pred_triplets[inds])
        #feature_list.append(feature[inds])
    pred_list = np.array(pred_list)
    #feature_list = np.array(feature_list)
    return pred_list


def save_results(gt_rels, gt_classes, gt_boxes, pred_rels, pred_classes, pred_boxes, pred_scores, obj_scores):
    ...
    return gt_triplets, predict_triplets
...
class SGStagewiseRecall(SceneGraphEvaluation):
    def __init__(
        ...
        self.vaild_rel_prop_num = cfg.MODEL.ROI_RELATION_HEAD.PISA_MODULE.MP_VALID_PAIRS_NUM

        self.mp_pair_refine_iter = 1
        if cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == "PISAPredictor":
            self.mp_pair_refine_iter = (
                cfg.MODEL.ROI_RELATION_HEAD.PISA_MODULE.MESSAGE_PROPOGATION_ITERATION_NUM
            )
        elif cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == "BGNNPredictor":
            self.mp_pair_refine_iter = (
