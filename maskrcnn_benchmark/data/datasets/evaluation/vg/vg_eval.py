...
from maskrcnn_benchmark.data.datasets.evaluation.vg.sgg_eval import SGRecall, SGNoGraphConstraintRecall, \
    SGZeroShotRecall, SGPairAccuracy, SGMeanRecall, SGStagewiseRecall, SGNGMeanRecall, save_results, _triplet
...
def do_vg_evaluation(
    ...
    zeroshot_triplet = torch.load("/home/sylvia/yjr/sgg/benchmark38/PySGG/maskrcnn_benchmark/data/datasets/evaluation/vg/zeroshot_triplet.pytorch",
    ...
    if "relations" in iou_types:
        ...
        relation_gt = []
        feature_predicted = []
        top20_relation_predicted = []
        #with open(os.path.join(cfg.OUTPUT_DIR, 'feature.pkl'), 'rb') as f:
        #    feature_all = pickle.load(f)
        for groundtruth, prediction in tqdm(zip(groundtruths, predictions), total=len(predictions)):
            evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator, relation_predicted, relation_gt)

        if mode == 'predcls':
            d = []
            d.append(dataset.filenames)
            d.append(relation_predicted)
            d.append(relation_gt)
            d.append(feature_predicted)
            with open(os.path.join(cfg.OUTPUT_DIR, 'relation.pkl'), 'wb') as f:
                pickle.dump(d, f)
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator, relation_pred, relation_gt):
    """
    Returns:
    ...
    if mode == 'predcls':
        ...
        gt_triplets, predicted_triplets = save_results(gt_rels, gt_classes, gt_boxes, pred_rels, pred_classes,
                                                       pred_boxes,
                                                       pred_scores, obj_scores)
        relation_pred.append(predicted_triplets)
        relation_gt.append(gt_triplets)
        #feature_pred.append(feature_list)
