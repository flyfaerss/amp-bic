...
from maskrcnn_benchmark.structures.boxlist_ops import boxlist_iou, split_boxlist, cat_boxlist
from maskrcnn_benchmark.utils.comm import get_rank, synchronize

#from maskrcnn_benchmark.data.datasets.fa_inst_rel_rsmp import resampling_dict_generation, apply_resampling
...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        self.predicate_dir = os.path.join(cfg.OUTPUT_DIR, f'rel_informative.txt')
        self.use_init_frequency = cfg.MODEL.ROI_RELATION_HEAD.PISA_MODULE.USE_INIT_FREQUENCY
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.DATA_RESAMPLING and self.split == 'train' and self.resampling_method in ['bilvl',
                                                                                                                'lvis']:
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.DATA_RESAMPLING and self.split == 'train' and self.resampling_method == 'faip':
            # creat repeat dict in main process, other process just wait and load
            if get_rank() == 0:
                repeat_dict, rel_true_counter = resampling_dict_generation(self, self.ind_to_predicates,
                                                                           self.ind_to_classes, logger)
                self.repeat_dict = repeat_dict
                self.rel_true_counter = rel_true_counter
                with open(os.path.join(cfg.OUTPUT_DIR, "repeat_dict.pkl"), "wb") as f:
                    pickle.dump(self.repeat_dict, f)
                with open(os.path.join(cfg.OUTPUT_DIR, "times_list.pkl"), "wb") as f:
                    pickle.dump(self.rel_true_counter, f)

            synchronize()
            self.repeat_dict, self.rel_true_counter = resampling_dict_generation(self, self.ind_to_predicates,
                                                                                 self.ind_to_classes, logger)
            ...
            for idx in range(len(self.filenames)):
                r_c = self.repeat_dict[idx]
                duplicate_idx_list.extend([idx for _ in range(r_c)])'''
            self.idx_list = self.repeat_dict
    ...
    def get_statistics(self):
        fg_matrix, bg_matrix, rel_counter_init, inst_counter_init = get_VG_statistics(self,
                                                                                      must_overlap=True)
        eps = 1e-3
        bg_matrix += 1
        ...
        for i in tqdm(self.idx_list):
            ...
            instance_labels = np.array(torch.from_numpy(self.gt_classes[i]))

            if self.repeat_dict is not None:
                if self.resampling_method != 'faip':
                    relation, relation_non_masked = apply_resampling(i,
                                                                     relation,
                                                                     instance_labels,
                                                                     self.repeat_dict,
                                                                     self.drop_rate)
                    if last != i:
                        last = i
                        for rel in relation_non_masked:
                            if rel[-1] > 0:
                                rel_true_counter[rel[-1]] += 1
                else:
            ...
            for rel in relation:
                if rel[-1] > 0:
                    rel_counter[rel[-1]] += 1
                    inst_counter[instance_labels[rel[0]]] += 1
                    inst_counter[instance_labels[rel[1]]] += 1
        ...
        F_c = np.zeros(50)
        if self.use_init_frequency:
            for k in range(50):
                F_c[k] = rel_counter_init[k + 1]
        else:
            if self.repeat_dict is not None:
                if self.resampling_method != 'faip':
                    for k in range(50):
                        F_c[k] = rel_true_counter[k + 1]
                else:
                    for k in range(50):
                        F_c[k] = self.rel_true_counter[k]
            else:
                for k in range(50):
                    F_c[k] = rel_counter[k + 1]
        total = sum(F_c)
        F_c /= (total + 1e-11)

        predicate_informative = -1 * np.log(F_c)

        with open(self.predicate_dir, 'w') as f:
            f.write(str(predicate_informative))
        f.close()

        sorted_cate_list = [i[0] for i in rel_counter_init.most_common()]
        inst_sorted_cate_list = [i[0] for i in inst_counter_init.most_common()]
        ...
        for cate_id in sorted_cate_list:
            ...
            rel_counter_name.append(self.ind_to_predicates[cate_id])  # list start from 0
            rel_cate_num.append(rel_counter[cate_id])  # dict start from 1
            rel_cate_num_init.append(rel_counter_init[cate_id])  # dict start from 1

        for inst_id in inst_sorted_cate_list:
            inst_counter_name.append(self.ind_to_classes[inst_id])
            inst_cate_num.append(inst_counter[inst_id])
            inst_cate_num_init.append(inst_counter_init[inst_id])
        ...
        axs_c[0].bar(rel_counter_name, rel_cate_num_init, color=color, width=0.6, zorder=0)
        axs_c[0].grid()
        plt.sca(axs_c[0])
        plt.xticks(rotation=-90, )
        for a, b, l in zip(rel_counter_name, rel_cate_num_init, range(len(rel_counter_name))):
            plt.text(a, b + 0.01, "%d" % rel_cate_num_init[l], ha='center', fontsize=12, rotation=-90)

        axs_c[1].bar(rel_counter_name, rel_cate_num, color=color, width=0.6, zorder=0)
        axs_c[1].grid()
        # axs_c[1].set_ylim(0, 50000)
        plt.sca(axs_c[1])
        plt.xticks(rotation=-90, )
        for a, b, l in zip(rel_counter_name, rel_cate_num, range(len(rel_counter_name))):
            plt.text(a, b + 0.1, "%d" % rel_cate_num[l], ha='center', fontsize=12, rotation=-90)

        save_file = os.path.join(cfg.OUTPUT_DIR, f"rel_freq_dist.png")
        fig.savefig(save_file, dpi=600)

        fig, axs_c = plt.subplots(2, 1, figsize=(25, 10), tight_layout=True)
        fig.set_facecolor((1, 1, 1))

        axs_c[0].bar(inst_counter_name, inst_cate_num_init, color='b', width=0.4, zorder=0)
        axs_c[0].grid()
        plt.sca(axs_c[0])
        plt.xticks(rotation=-90, )
        for a, b, l in zip(inst_counter_name, inst_cate_num_init, range(len(inst_counter_name))):
            plt.text(a, b + 0.1, "%d" % inst_cate_num_init[l], ha='center', fontsize=8, rotation=-90)

        axs_c[1].bar(inst_counter_name, inst_cate_num, color='b', width=0.4, zorder=0)
        axs_c[1].grid()
        # axs_c[1].set_ylim(0, 50000)
        plt.sca(axs_c[1])
        plt.xticks(rotation=-90, )
        for a, b, l in zip(inst_counter_name, inst_cate_num, range(len(inst_counter_name))):
            plt.text(a, b + 0.1, "%d" % inst_cate_num[l], ha='center', fontsize=8, rotation=-90)

        save_file = os.path.join(cfg.OUTPUT_DIR, f"inst_freq_dist.png")
        fig.savefig(save_file, dpi=600)
...
def get_VG_statistics(train_data, must_overlap=True):
    ...
    return fg_matrix, bg_matrix, rel_counter, inst_counter
...
def get_frequency_distribution(dir):
    file_path = os.path.join(dir, f'rel_informative.txt')
