...
def make_data_loader(cfg, mode='train', is_distributed=False, start_iter=0):
    ...
    s = []
    s.append(datasets[0].ind_to_classes)
    s.append(datasets[0].ind_to_predicates)
    s.append(datasets[0].filenames)
    s.append(datasets[0].gt_boxes)
    s.append(datasets[0].gt_classes)
    s.append(datasets[0].relationships)
    with open(os.path.join(cfg.OUTPUT_DIR, 'Dataset.pkl'), 'wb') as f:
        pickle.dump(s, f)
